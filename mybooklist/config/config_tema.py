...
def carregar_tema_salvo() -> str:
    """Lê o tema salvo em disco; retorna o padrão se o arquivo não existir,
    estiver corrompido ou o tema for desconhecido."""
    try:
        with open(ARQUIVO_CONFIG_TEMA, encoding="utf-8") as f:
            dados = json.load(f)
...
def salvar_tema(nome_tema: str) -> None:
    """Persiste a preferência de tema; em caso de falha de escrita, avisa e segue
    (a preferência só não sobrevive ao fechamento do app)."""
    try:
        with open(ARQUIVO_CONFIG_TEMA, "w", encoding="utf-8") as f:
            json.dump({"tema": nome_tema}, f)
    except OSError as erro:
        print(f"Aviso: não foi possível salvar a preferência de tema – {erro}")
